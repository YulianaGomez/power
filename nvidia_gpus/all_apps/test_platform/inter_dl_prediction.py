...
"""
# Used when doing itra-architecture prediction
...
path = '/Users/yzamora/power/nvidia_gpus/all_apps/large_dh_sets/'
X_train = np.load(path + 'XP_train.npy')
X_val = np.load(path + 'XP_val.npy')
y_train = np.load(path + 'yV_train.npy')
y_val = np.load(path + 'yV_val.npy')
XP_test = np.load(path + 'XP_test.npy')
yP_test = np.load(path + 'yV_test.npy')
...
p_model.fit(X_train, y_train, epochs=10000, batch_size=10000, verbose=1,validation_data=(X_val,y_val))
...
y_scaler = load('/Users/yzamora/power/nvidia_gpus/all_apps/large_dh_sets/std_scaler_y.bin')
...
train_set_per = 50
if not train_set_per:
    train_set_per = round((1 - train_test_size) * .7 * 100, 2)
points_used = X_train.shape[0]
#import pdb; pdb.set_trace()
st_points = "{:,d}".format(points_used)
plt.title("Inter-Architecture IPC prediction (P100) - MAPE: " + str(round(MAPE_score,2)) + ", Training Points Used: " + st_points)
