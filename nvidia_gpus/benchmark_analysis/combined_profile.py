...
exe_script = sys.argv[1]
results_dir = sys.argv[2]

print ("This is from arg first argument ./" + exe_script)
print ("This is from second argument " + results_dir)
...
#blocks = [128] #, 256, 512, 1024]
#for n_size in range(1000000,1000000000,10000000):
for n_size in range(100,1000,100):
    for block_size in blocks:
        #print("this is n_size", n_size)
        #results +='Memory Total \t Memory Used \t    Memory Free \t Power Drawed \t Clocks \t \n'
        combined_results = open(results_dir + "/combined.n" + str(n_size) + "b" + str(block_size) + ".results",'a')
        combined_rate = open(results_dir + "/combined.n" + str(n_size) + "b" + str(block_size) + ".rate",'a')
        ...
        pargs = ["./" + exe_script + " -B %s -N %s" % (str(block_size),str(n_size))]
        ##print (pargs)
