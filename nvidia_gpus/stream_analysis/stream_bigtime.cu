...
/*	--- MAIN LOOP --- repeat test cases NTIMES times --- */
 ...
 for (k=0; k<NTIMES; k++)
 {
  ...
  //times[3][k]= mysecond() -  times[3][k];
   ...
   coarse_time = mysecond() - coarse_time;
...
/*	--- SUMMARY --- */
	...
	for (j=0; j<4; j++)
	    {
	    avgtime[j] = coarse_time; //avgtime[j] + times[j][k];
	    mintime[j] = coarse_time; //MIN(mintime[j], times[j][k]);
	    maxtime[j] = 1; //MAX(maxtime[j], times[j][k]);
	    }
	}
